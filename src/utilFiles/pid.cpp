...
void PIDMove(double desiredX, double desiredY, bool isReverse) {
  ...
  */
  while (odomLoopActive) pros::delay(1);
  PIDActive = true;
  ...
  */
  if (isReverse) desiredOrientation -= CONST_PI;
...
void PIDTurn(double desiredOrientation) {
  ...
  */
  ...
  while (fabs(error) > 0.01) {
    ...
    */
    if (error > 0 && error <= CONST_PI) error = error;
    else if (error > 0 && error > CONST_PI) error = ((2 * CONST_PI) - error) * -1;
    else if (error < 0 && fabs(error) <= CONST_PI) error = error;
    else if (error < 0 && fabs(error) > CONST_PI) error = simplifyRadAngle((2 * CONST_PI) - error);
